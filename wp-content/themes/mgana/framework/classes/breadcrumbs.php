<?php
/**
 * Mgana Breadcrumb Trail class
 *
 * @package Mgana WordPress theme
 * @since 1.0.0
 */

if ( !function_exists('mgana_breadcrumb_trail') ) {
    function mgana_breadcrumb_trail( $args = array() ) {


        // Return if breadcrumbs are disabled
        if ( ! mgana_has_breadcrumbs() || is_front_page() ) {
            return;
        }
...
if( !function_exists('mgana_sp_breadcrumbs_before') ) {
    function mgana_sp_breadcrumbs_before() {
        $classes = 'site-breadcrumbs';
        echo '<div class="'. esc_attr($classes) .'">';
    }
}
add_action( 'seopress_breadcrumbs_before_html', 'mgana_sp_breadcrumbs_before' );

if(!function_exists('mgana_sp_breadcrumbs_after')){
    function mgana_sp_breadcrumbs_after() {
        echo '</div>';
    }
}
add_action( 'seopress_breadcrumbs_after_html', 'mgana_sp_breadcrumbs_after' );
...
if( !class_exists('Mgana_Breadcrumb_Trail') ) {

    class Mgana_Breadcrumb_Trail {
        ...
        /**
         * Sets up the breadcrumb trail properties.  Calls the `Mgana_Breadcrumb_Trail::add_items()` method
         ...
         *     @type array     $labels         Text labels. @see Mgana_Breadcrumb_Trail::set_labels()
         *     @type array     $post_taxonomy  Taxonomies to use for post types. @see Mgana_Breadcrumb_Trail::set_post_taxonomy()
        ...
        public function get_trail() {
            ...
            $breadcrumb    	= '';
            $separator      = apply_filters( 'breadcrumb_trail_separator', mgana_get_option('breadcrumb_separator', '>') );
            ...
            if ( 0 < $item_count ) {
                ...
                $breadcrumb .= '<ul class="trail-items"'. mgana_get_schema_markup( 'breadcrumb' ) .'>';
                ...
                foreach ( $this->items as $item ) {
                    ...
                    $item = !empty( $matches ) ? sprintf( '%s<span'. mgana_get_schema_markup( 'author_name' ) .'>%s</span>%s', $matches[1], $matches[2], $matches[3] ) : sprintf( '<span'. mgana_get_schema_markup( 'author_name' ) .'>%s</span>', $item );
                    ...
                    $attributes = 'class="' . $item_class . '"'. mgana_get_schema_markup( 'breadcrumb_list' ) .'';

                    // Separator
                    if ( $item_count === $item_position ) {
                        $sep = '';
                        $current_url = home_url(add_query_arg(null, null));
                        $tmp = sprintf( '<meta content="%s"'. mgana_get_schema_markup( 'item' ) .' />', $current_url );
                        $item .= $tmp;
                    } else {
                        $sep = $separator;
                    }
                    ...
                    $meta = sprintf( '<meta content="%s"'. mgana_get_schema_markup( 'position' ) .' />', absint( $item_position ) );
                ...
                $breadcrumb = sprintf(
                    '<%1$s aria-label="%2$s" class="site-breadcrumbs'. $p_class .'"'. mgana_get_schema_markup( 'breadcrumb_itemprop' ) .'>%3$s%4$s%5$s</%1$s>',
        ...
        public function trail() {
            ...
            echo mgana_render_variable( $this->get_trail() );
        }
        ...
        protected function set_labels() {
            ...
            $defaults = array(
                'aria_label'          	=> esc_attr_x( 'Breadcrumbs', 'breadcrumbs aria label', 'mgana' ),
                'home'           		=> mgana_get_option( 'breadcrumb_translation_home', esc_html__( 'Home', 'mgana' ) ),
                'error_404'           	=> mgana_get_option( 'breadcrumb_translation_error', esc_html__( '404 Not Found', 'mgana' ) ),
                'archives'            	=> esc_html__( 'Archives',                              'mgana' ),
                // Translators: %s is the search query. The HTML entities are opening and closing curly quotes.
                'search'           		=> mgana_get_option( 'breadcrumb_translation_search', esc_html__( 'Search results for', 'mgana' ) ),
                // Translators: %s is the page number.
                'paged'               	=> esc_html__( 'Page %s',                               'mgana' ),
                // Translators: Minute archive title. %s is the minute time format.
                'archive_minute'      	=> esc_html__( 'Minute %s',                             'mgana' ),
                // Translators: Weekly archive title. %s is the week date format.
                'archive_week'        	=> esc_html__( 'Week %s',                               'mgana' ),
        ...
        protected function add_network_home_link() {
            ...
            // Home item
            $setting = mgana_get_option( 'breadcrumb_home_item', 'icon' );
        ...
        protected function add_singular_items() {
            ...
            // Post type taxonomy
            $posts_tax 		= mgana_get_option( 'breadcrumb_posts_taxonomy', 'category' );
            $products_tax 	= mgana_get_option( 'breadcrumb_products_taxonomy', 'shop' );
            $portfolio_tax 	= mgana_get_option( 'breadcrumb_portfolio_taxonomy', 'la_portfolio_category' );
            ...
            // If the post type is 'lastudio_portfolio'.
                ...
                $portfolio_id = mgana_get_option( 'portfolio_page' );
        ...
        protected function add_minute_hour_archive_items() {
            ...
            if ( true === $this->args['show_title'] )
                $this->items[] = sprintf( $this->labels['archive_minute_hour'], get_the_time( esc_html_x( 'g:i a', 'minute and hour archives time format', 'mgana' ) ) );
        }
        ...
        protected function add_minute_archive_items() {
            ...
            // Add the minute item.
            if ( true === $this->args['show_title'] )
                $this->items[] = sprintf( $this->labels['archive_minute'], get_the_time( esc_html_x( 'i', 'minute archives time format', 'mgana' ) ) );
        }
        ...
        protected function add_hour_archive_items() {
            ...
            // Add the hour item.
            if ( true === $this->args['show_title'] )
                $this->items[] = sprintf( $this->labels['archive_hour'], get_the_time( esc_html_x( 'g a', 'hour archives time format', 'mgana' ) ) );
        }
        ...
        protected function add_day_archive_items() {
            ...
            $year  = sprintf( $this->labels['archive_year'],  get_the_time( esc_html_x( 'Y', 'yearly archives date format',  'mgana' ) ) );
            $month = sprintf( $this->labels['archive_month'], get_the_time( esc_html_x( 'F', 'monthly archives date format', 'mgana' ) ) );
            $day   = sprintf( $this->labels['archive_day'],   get_the_time( esc_html_x( 'j', 'daily archives date format',   'mgana' ) ) );
        ...
        protected function add_week_archive_items() {
            ...
            $year = sprintf( $this->labels['archive_year'],  get_the_time( esc_html_x( 'Y', 'yearly archives date format', 'mgana' ) ) );
            $week = sprintf( $this->labels['archive_week'],  get_the_time( esc_html_x( 'W', 'weekly archives date format', 'mgana' ) ) );
        ...
        protected function map_rewrite_tags( $post_id, $path ) {
            ...
            if ( is_array( $matches ) ) {
                ...
                foreach ( $matches as $match ) {
                    ...
                    if ( '%year%' == $tag )
                        $this->items[] = sprintf( '<a href="%s" itemprop="item">%s</a>', esc_url( get_year_link( get_the_time( 'Y', $post_id ) ) ), sprintf( $this->labels['archive_year'], get_the_time( esc_html_x( 'Y', 'yearly archives date format',  'mgana' ) ) ) );

                    // If using the %monthnum% tag, add a link to the monthly archive.
                    elseif ( '%monthnum%' == $tag )
                        $this->items[] = sprintf( '<a href="%s" itemprop="item">%s</a>', esc_url( get_month_link( get_the_time( 'Y', $post_id ), get_the_time( 'm', $post_id ) ) ), sprintf( $this->labels['archive_month'], get_the_time( esc_html_x( 'F', 'monthly archives date format', 'mgana' ) ) ) );

                    // If using the %day% tag, add a link to the daily archive.
                    elseif ( '%day%' == $tag )
                        $this->items[] = sprintf( '<a href="%s" itemprop="item">%s</a>', esc_url( get_day_link( get_the_time( 'Y', $post_id ), get_the_time( 'm', $post_id ), get_the_time( 'd', $post_id ) ) ), sprintf( $this->labels['archive_day'], get_the_time( esc_html_x( 'j', 'daily archives date format', 'mgana' ) ) ) );
