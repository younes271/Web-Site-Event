...
if (!function_exists('curly_mkdf_google_fonts_styles')) {
    ...
    function curly_mkdf_google_fonts_styles() {
        $is_enabled = boolval( apply_filters( 'curly_mkdf_filter_enable_google_fonts', true ) );
...
if (!function_exists('curly_mkdf_scripts')) {
    ...
    function curly_mkdf_scripts() {
        ...
        if (!empty($google_maps_api_key)) {
            wp_enqueue_script('curly-mkdf-google-map-api', '//maps.googleapis.com/maps/api/js?key=' . esc_attr($google_maps_api_key) . "&callback=mkdfEmptyCallback" . $google_maps_extensions, array(), false, true);
            if ( ! empty( $google_maps_extensions_array ) && is_array( $google_maps_extensions_array ) ) {
                wp_enqueue_script('jquery.geocomplete', get_template_directory_uri() . '/framework/admin/assets/js/jquery.geocomplete.min.js', array('curly-mkdf-google-map-api'), false, true);
            }
            wp_add_inline_script('curly-mkdf-google-map-api', 'window.mkdfEmptyCallback = function () {};','before');
        }
