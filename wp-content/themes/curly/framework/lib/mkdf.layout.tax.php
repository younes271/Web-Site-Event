...
/*
   Class: CurlyMikadofTaxonomyField
   A class that initializes CurlyMikadof Taxonomy Field
*/

class CurlyMikadofTaxonomyField implements iCurlyMikadofRender
{
    private $type;
    private $name;
    private $label;
    ...
    function __construct($type, $name, $label = "", $description = "", $options = array(), $args = array(), $hidden_property = "", $hidden_values = array()) {
        ...
        add_filter('curly_mkdf_taxonomy_fields', array($this, 'addFieldForEditSave'));
    }
...
abstract class CurlyMikadofTaxonomyFieldType
{
    abstract public function render($name, $label = "", $description = "", $options = array(), $args = array());
}

class CurlyMikadofTaxonomyFieldText extends CurlyMikadofTaxonomyFieldType
{
...
class CurlyMikadofTaxonomyFieldImage extends CurlyMikadofTaxonomyFieldType
{
    public function render($name, $label = "", $description = "", $options = array(), $args = array(), $hidden = false) {
        if (!isset($_GET['tag_ID'])) { ?>
            <div class="form-field">
                ...
                <p>
                    <input type="button" class="button button-secondary mkdf-tax-media-add" name="mkdf-tax-media-add" value="<?php esc_attr_e('Add Image', 'curly'); ?>"/>
                    <input type="button" class="button button-secondary mkdf-tax-media-remove" name="mkdf-tax-media-remove" value="<?php esc_attr_e('Remove Image', 'curly'); ?>"/>
                </p>
            </div>
            <?php
        } else {
            ...
            <tr class="form-field" <?php if ($hidden) { ?> style="display: none"<?php } ?>>
                ...
                <td>
                    ...
                    <p>
                        <input type="button" class="button button-secondary mkdf-tax-media-add" name="mkdf-tax-media-add" value="<?php esc_attr_e('Add Image', 'curly'); ?>"/>
                        <input data-termid="<?php echo esc_html($_GET['tag_ID']); ?>" data-taxonomy="<?php echo esc_html($taxonomy); ?>" type="button" class="button button-secondary mkdf-tax-media-remove" name="mkdf-tax-media-remove" value="<?php esc_attr_e('Remove Image', 'curly'); ?>"/>
                    </p>
                </td>
            </tr>
            <?php
...
class CurlyMikadofTaxonomyFieldSelect extends CurlyMikadofTaxonomyFieldType
{
    public function render($name, $label = "", $description = "", $options = array(), $args = array(), $hidden = false) {
        ...
        if (!isset($_GET['tag_ID'])) { ?>
            <div class="form-field">
                ...
                <select
                    class="<?php echo esc_attr($select2) ?> form-control mkdf-form-element<?php if ($dependence) {
                        echo " dependence";
                    } ?>"
            ...
            <tr class="form-field" <?php if ($hidden) { ?> style="display: none"<?php } ?>>
                ...
                <td>
                    <select name="<?php echo esc_attr($name); ?>"
                        class="<?php echo esc_attr($select2) ?> mkdf-form-element<?php if ($dependence) {
                            echo " dependence";
                        } ?>"
                        <?php foreach ($show as $key => $value) { ?>
                            data-show-<?php echo str_replace(' ', '', $key); ?>="<?php echo esc_attr($value); ?>"
...
class CurlyMikadofTaxonomyFieldIcon extends CurlyMikadofTaxonomyFieldType
{
    public function render($name, $label = "", $description = "", $options = array(), $args = array(), $hidden = false) {
        ...
        if (!isset($_GET['tag_ID'])) { ?>
            ...
            <?php foreach ($icons_collections as $icons_collection) {
                ...
                <div class="form-field mkd-icon-collection-holder" style="display: none" data-icon-collection="<?php echo esc_attr($icons_collection); ?>">
                    <label for="<?php echo esc_attr($name) . '_icon'; ?>"><?php esc_html_e('Icon', 'curly'); ?></label>
            ...
            <?php foreach ($icons_collections as $icons_collection) {
                ...
                <tr class="form-field mkd-icon-collection-holder" style="<?php echo esc_attr($style); ?>" data-icon-collection="<?php echo esc_attr($icons_collection); ?>">
                    <th scope="row"><?php esc_html_e('Icon', 'curly'); ?></th>
                    <td>
                        <select name="<?php echo esc_attr($name . '_' . $icons_param) ?>" id="<?php echo esc_attr($name . '_' . $icons_param) ?>">
                            <?php
                            $icons = \CurlyMikadofIconCollections::get_instance()->getIconCollection($icons_collection);
...
class CurlyMikadofTaxonomyFieldColor extends CurlyMikadofTaxonomyFieldType
{
    public function render($name, $label = "", $description = "", $options = array(), $args = array(), $hidden = false) {
...
class CurlyMikadofTaxonomyFieldFactory
{
    public function render($field_type, $name, $label = "", $description = "", $options = array(), $args = array(), $hidden = false) {

        switch (strtolower($field_type)) {
            case 'text':
                $field = new CurlyMikadofTaxonomyFieldText();
                $field->render($name, $label, $description, $options, $args, $hidden);
                break;

            case 'image':
                $field = new CurlyMikadofTaxonomyFieldImage();
                $field->render($name, $label, $description, $options, $args, $hidden);
                break;

            case 'selectblank':
                $field = new CurlyMikadofTaxonomyFieldSelect();
                $field->render($name, $label, $description, $options, $args, $hidden);
                break;

            case 'icon':
                $field = new CurlyMikadofTaxonomyFieldIcon();
                $field->render($name, $label, $description, $options, $args, $hidden);
                break;

            case 'color':
                $field = new CurlyMikadofTaxonomyFieldColor();
                $field->render($name, $label, $description, $options, $args, $hidden);
                break;
