...
class MikadoSkin extends CurlyMikadofSkinAbstract
{
    /**
     * Skin constructor. Hooks to curly_mkdf_admin_scripts_init and curly_mkdf_enqueue_admin_styles
     */
    public function __construct() {
        $this->skinName = 'mikado';

        //hook to
        add_action('curly_mkdf_admin_scripts_init', array($this, 'registerStyles'));
        add_action('curly_mkdf_admin_scripts_init', array($this, 'registerScripts'));

        add_action('curly_mkdf_enqueue_admin_styles', array($this, 'enqueueStyles'));
        add_action('curly_mkdf_enqueue_admin_scripts', array($this, 'enqueueScripts'));

        add_action('curly_mkdf_enqueue_meta_box_styles', array($this, 'enqueueStyles'));
        add_action('curly_mkdf_enqueue_meta_box_scripts', array($this, 'enqueueScripts'));
    ...
    public function registerScripts() {
        ...
        foreach ($this->scripts as $scriptHandle => $script) {
            curly_mkdf_register_skin_script($scriptHandle, $script['path'], $script['dependency']);
        }
    }
    ...
    public function registerStyles() {
        ...
        foreach ($this->styles as $styleHandle => $stylePath) {
            curly_mkdf_register_skin_style($styleHandle, $stylePath);
        }
    }
    ...
    public function renderOptions() {
        ...
        $tab = curly_mkdf_get_admin_tab();
        $active_page = $curly_mkdf_Framework->mkdOptions->getAdminPageFromSlug($tab);
     ...
     *
     * @see CurlyMikadofSkinAbstract::loadTemplatePart()
     */
     ...
     *
     * @param CurlyMikadofAdminPage $page current page to load
    ...
    public function renderBackupOptions() { ?>
        <div class="mkdf-options-page mkdf-page">
            <?php $this->getHeader(curly_mkdf_get_theme_info_item('Name'), curly_mkdf_get_theme_info_item('Version'), false); ?>
            <div class="mkdf-page-content-wrapper">
                <div class="mkdf-page-content">
                    <div class="mkdf-page-navigation mkdf-tabs-wrapper vertical left clearfix">
