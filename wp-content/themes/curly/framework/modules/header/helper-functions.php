...
if (!function_exists('curly_mkdf_header_skin_class')) {
    /**
     * Function that adds header style class to body tag
     */
    function curly_mkdf_header_skin_class($classes) {
        $header_style = curly_mkdf_get_meta_field_intersect('header_style', curly_mkdf_get_page_id());
        $header_style_404 = curly_mkdf_options()->getOptionValue('404_header_style');
    ...
    add_filter('body_class', 'curly_mkdf_header_skin_class');
}

if (!function_exists('curly_mkdf_sticky_header_behaviour_class')) {
    /**
     * Function that adds header behavior class to body tag
     */
    function curly_mkdf_sticky_header_behaviour_class($classes) {
        $header_behavior = curly_mkdf_get_meta_field_intersect('header_behaviour', curly_mkdf_get_page_id());
    ...
    add_filter('body_class', 'curly_mkdf_sticky_header_behaviour_class');
}

if (!function_exists('curly_mkdf_menu_dropdown_appearance')) {
    /**
     ...
     */
    function curly_mkdf_menu_dropdown_appearance($classes) {
        $dropdown_menu_appearance = curly_mkdf_options()->getOptionValue('menu_dropdown_appearance');
    ...
    add_filter('body_class', 'curly_mkdf_menu_dropdown_appearance');
}

if (!function_exists('curly_mkdf_header_class')) {
    /**
     ...
     */
    function curly_mkdf_header_class($classes) {
        ...
        $disable_menu_area_shadow = curly_mkdf_get_meta_field_intersect('menu_area_shadow', $id) == 'no';
        if ($disable_menu_area_shadow) {
            $classes[] = 'mkdf-menu-area-shadow-disable';
        }

        $disable_menu_area_grid_shadow = curly_mkdf_get_meta_field_intersect('menu_area_in_grid_shadow', $id) == 'no';
        if ($disable_menu_area_grid_shadow) {
            $classes[] = 'mkdf-menu-area-in-grid-shadow-disable';
        }

        $disable_menu_area_border = curly_mkdf_get_meta_field_intersect('menu_area_border', $id) == 'no';
        if ($disable_menu_area_border) {
            $classes[] = 'mkdf-menu-area-border-disable';
        }

        $disable_menu_area_grid_border = curly_mkdf_get_meta_field_intersect('menu_area_in_grid_border', $id) == 'no';
        if ($disable_menu_area_grid_border) {
            $classes[] = 'mkdf-menu-area-in-grid-border-disable';
        }

        if (curly_mkdf_get_meta_field_intersect('menu_area_in_grid', $id) == 'yes' &&
            curly_mkdf_get_meta_field_intersect('menu_area_grid_background_color', $id) !== '' &&
            curly_mkdf_get_meta_field_intersect('menu_area_grid_background_transparency', $id) !== '0'
        ) {
            $classes[] = 'mkdf-header-menu-area-in-grid-padding';
        }

        $disable_logo_area_border = curly_mkdf_get_meta_field_intersect('logo_area_border', $id) == 'no';
        if ($disable_logo_area_border) {
            $classes[] = 'mkdf-logo-area-border-disable';
        }

        $disable_logo_area_grid_border = curly_mkdf_get_meta_field_intersect('logo_area_in_grid_border', $id) == 'no';
        if ($disable_logo_area_grid_border) {
            $classes[] = 'mkdf-logo-area-in-grid-border-disable';
        }

        if (curly_mkdf_get_meta_field_intersect('logo_area_in_grid', $id) == 'yes' &&
            curly_mkdf_get_meta_field_intersect('logo_area_grid_background_color', $id) !== '' &&
            curly_mkdf_get_meta_field_intersect('logo_area_grid_background_transparency', $id) !== '0'
        ) {
            $classes[] = 'mkdf-header-logo-area-in-grid-padding';
        }

        $disable_shadow_vertical = curly_mkdf_get_meta_field_intersect('vertical_header_shadow', $id) == 'no';
        if ($disable_shadow_vertical) {
            $classes[] = 'mkdf-header-vertical-shadow-disable';
        }

        $disable_border_vertical = curly_mkdf_get_meta_field_intersect('vertical_header_border', $id) == 'no';
    ...
    add_filter('body_class', 'curly_mkdf_header_class');
}

if (!function_exists('curly_mkdf_header_area_style')) {
    /**
     * Function that return styles for header area
     */
    function curly_mkdf_header_area_style($style) {
        ...
        $menu_area_background_color_rgba = curly_mkdf_rgba_color($menu_area_background_color, $menu_area_background_transparency);
        ...
        if ($container_side_padding !== '') {
            ...
            $current_style .= curly_mkdf_dynamic_css($menu_container_selector, $menu_container_styles);
        }
        ...
        $menu_area_grid_background_color_rgba = curly_mkdf_rgba_color($menu_area_grid_background_color, $menu_area_grid_background_transparency);

        if (!empty($menu_area_grid_background_color_rgba)) {
            $menu_area_grid_style['background-color'] = $menu_area_grid_background_color_rgba;
        }

        $current_style .= curly_mkdf_dynamic_css($menu_area_selector, $menu_area_style);
        $current_style .= curly_mkdf_dynamic_css($menu_area_grid_selector, $menu_area_grid_style);
        ...
        if ($dropdown_top_position !== '') {
            $dropdown_styles['top'] = curly_mkdf_filter_suffix($dropdown_top_position, '%') . '%';
        }

        $dropdown_selector = array($class_prefix . ' .mkdf-page-header .mkdf-drop-down .second');

        $current_style .= curly_mkdf_dynamic_css($dropdown_selector, $dropdown_styles);
        ...
        $logo_area_background_color_rgba = curly_mkdf_rgba_color($logo_area_background_color, $logo_area_background_transparency);
        ...
        $logo_area_grid_background_color_rgba = curly_mkdf_rgba_color($logo_area_grid_background_color, $logo_area_grid_background_transparency);
        ...
        if (!empty($logo_area_style)) {
            $current_style .= curly_mkdf_dynamic_css($logo_area_selector, $logo_area_style);
        }

        if (!empty($logo_area_grid_style)) {
            $current_style .= curly_mkdf_dynamic_css($logo_area_grid_selector, $logo_area_grid_style);
        }

        $current_style = apply_filters('curly_mkdf_add_header_page_custom_style', $current_style, $class_prefix, $page_id) . $style;

        return $current_style;
    }

    add_filter('curly_mkdf_add_page_custom_style', 'curly_mkdf_header_area_style');
}