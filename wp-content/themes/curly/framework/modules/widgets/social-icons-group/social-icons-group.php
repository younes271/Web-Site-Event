...
if ( class_exists( 'CurlyCoreClassWidget' ) ) {
	...
	class CurlyMikadofClassIconsGroupWidget extends CurlyCoreClassWidget {
		public function __construct() {
			parent::__construct(
				'mkdf_social_icons_group_widget',
				esc_html__( 'Curly Social Icons Group Widget', 'curly' ),
				array( 'description' => esc_html__( 'Use this widget to add a group of up to 6 social icons to a widget area.', 'curly' ) )
			);
		...
		protected function setParams() {
			$this->params = array_merge(
				...
				curly_mkdf_icon_collections()->getSocialIconWidgetMultipleParamsArray( 6 ),
				array(
					//array(
					//    'type' => 'dropdown',
					//    'name' => 'layout',
					//    'title' => esc_html__('Icons Layout', 'curly'),
					//    'options' => array(
					//        '' => esc_html__('Default', 'curly'),
					//        'square-icons' => esc_html__('Square', 'curly'),
					//    )
					//),
					array(
						'type'        => 'dropdown',
						'name'        => 'skin',
						'title'       => esc_html__( 'Square Icons Skin', 'curly' ),
						'description' => esc_html__( 'This applies to the square layout', 'curly' ),
						'options'     => array(
							''           => esc_html__( 'Dark Skin', 'curly' ),
							'light-skin' => esc_html__( 'Light Skin', 'curly' ),
						)
					),
					array(
						'type'    => 'dropdown',
						'name'    => 'alignment',
						'title'   => esc_html__( 'Text Alignment', 'curly' ),
						'options' => array(
					...
					array(
						'type'  => 'textfield',
						'name'  => 'icon_size',
						'title' => esc_html__( 'Icons Size (px)', 'curly' )
					),
					array(
						'type'  => 'colorpicker',
						'name'  => 'color',
		...
		public function widget( $args, $instance ) {
			...
			if ( ! empty( $instance['icon_size'] ) ) {
				$icon_styles[] = 'font-size: ' . curly_mkdf_filter_px( $instance['icon_size'] ) . 'px';
			}
			...
			for ( $n = 1; $n <= 6; $n ++ ) {
				...
				<?php if ( ! empty( $icon_holder_html ) ) { ?>
					<a class="mkdf-social-icon-widget-holder mkdf-icon-has-hover" <?php echo curly_mkdf_get_inline_attr( $hover_color, 'data-hover-color' ); ?> <?php curly_mkdf_inline_style( $icon_styles ) ?>
