...
add_action('init', 'apr_like_scripts');

add_action('wp_ajax_nopriv_apr-post-like', 'apr_post_like');
add_action('wp_ajax_apr-post-like', 'apr_post_like');

function apr_post_like() {
    $nonce = $_POST['nonce'];
    if (!wp_verify_nonce($nonce, 'ajax-nonce'))
        die('Nope!');

    if (isset($_POST['apr_post_like'])) {
        $like_type = 'post';
        ...
        if (is_user_logged_in()) { // user is logged in
            ...
            if (!apr_AlreadyLiked($post_id)) { // like the post
                if($like_type == 'comment') {
...
function apr_AlreadyLiked($post_id) { // test if user liked before
    $like_type = 'post';
    if (isset($_POST['like_type']) && !empty($_POST['like_type'])) {
        $like_type = $_POST['like_type'];
    }
...
function apr_getPostLikeLink($post_id, $like_type = 'post') {
    if ($like_type == 'comment') {
    ...
    $output = '<a href="#" class="apr-post-like' . $class . '"  data-like_type="'.$like_type.'" data-post_id="' . $post_id . '" title="' . $title . '">' . $heart . $count . '</a>';
    return $output;
}
