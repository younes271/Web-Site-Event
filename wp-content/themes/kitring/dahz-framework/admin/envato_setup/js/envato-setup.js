...
var EnvatoWizard = (function($){
    ...
    function PluginManager(){
        ...
        function ajax_callback(response){
            if( typeof response == 'string' || ( typeof response == 'object' && typeof response.message != 'undefined' ) ){
                $current_node.find('span').text(response.message);
                if( typeof response.url != 'undefined' || typeof response == 'string' ){
                ...
                }else if( typeof response == 'string' || ( typeof response == 'object' && typeof response.done != 'undefined' ) ){
    ...
    function ContentManager(){
        ...
        function process_current(){
            if(current_item){
                ...
                var $check = $current_node.find('input:checkbox'), subContent;
                if($check.is(':checked')) {
                    console.log("Doing 2 "+current_item);
                    // process htis one!
					subContent = [];
					$('tr.envato_default_sub_content .envato_default_sub_content:checked').each(function(){
						subContent.push(this.value)
					});
                    jQuery.post(envato_setup_params.ajaxurl, {
    ...
    function toggleDetail() {
        $(document).on('click', '.envato_default_sub_content .toggle-header', function() {
...
(function($){
	var $ul, $li, style, isLocalFile; 
	$( document ).on( 'click', '.dahz-theme-presets a', function(e) {
		e.preventDefault();
		...
		isLocalFile = $(this).data('is-local-file');
		$('#style').val(style);
		$('#is_local_file').val(isLocalFile);
		return false;
	});
})(jQuery);
(function($){
	$( document ).on('ready',function(){
		var $parent;
		$('tr.envato_default_content input.envato_default_content').on('change',function(){
			$parent = $( this ).parents('tr.envato_default_content');
			if( this.checked ){
				$('tr.envato_default_sub_content[data-sub-content="'+$parent.attr('data-content')+'"] input.envato_default_sub_content').attr('checked','checked');
			} else {
				$('tr.envato_default_sub_content[data-sub-content="'+$parent.attr('data-content')+'"] input.envato_default_sub_content').removeAttr('checked');
			}
		});
		$('tr.envato_default_content input.envato_default_content').each(function(){
			$parent = $( this ).parents('tr.envato_default_content');
			if( this.checked ){
				$('tr.envato_default_sub_content[data-sub-content="'+$parent.attr('data-content')+'"] input.envato_default_sub_content').attr('checked','checked');
			} else {
		...
		$('tr.envato_default_sub_content input.envato_default_sub_content').on('change',function(){
			$parent = $( this ).parents('tr.envato_default_sub_content');
			if( $( 'input.envato_default_sub_content:checked', $parent ).length ){
				$('tr.envato_default_content[data-content="'+$parent.attr('data-sub-content')+'"] input.envato_default_content').attr('checked','checked');
			} else {
				$('tr.envato_default_content[data-content="'+$parent.attr('data-sub-content')+'"] input.envato_default_content').removeAttr('checked');
			}
		});
	})
